...
if __name__ == '__main__':
    ...
    parser.add_argument('--net', type = str, choices=['LeNet5', 'AlexNet', 'VGG16','VGG19','ResNet18','ResNet34',   
                                                       'DenseNet','MobileNetv1','MobileNetv2'], default='VGG19', help='net type')
    ...
    if args.net == 'VGG16':
        ...
        from nets.VGG import VGG
        net = VGG('VGG19')
        from nets.teacher.VGG import VGG
        net_T = VGG('VGG19')
    ...
    def train(epoch):
        ...
        with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{epochs}',postfix=dict,mininterval=0.3) as pbar:
            for step,(im,label) in enumerate(trainloader,start=0):
                ...
                #   计算损失
                #----------------------#
                loss_1 = 0.2*criterion(outputs_1,label)
                loss_2 = 0.2*criterion(outputs_2,label)
                loss_3 = 0.2*criterion(outputs_3,label)
                loss_4 = 0.2 * criterion(outputs_4, label)
                loss = 0.2*criterion(outputs,label)

                loss_s = loss+loss_1+loss_2+loss_3+loss_4
                ...
                outputs_diss_4 = F.log_softmax(outputs_4 / T, dim=1)
                ...
                loss_t_4 = criterion2(outputs_diss_4, outputs_T) * T * T

                loss_t = loss_t_1 + loss_t_2 + loss_t_3+ loss_t_4
